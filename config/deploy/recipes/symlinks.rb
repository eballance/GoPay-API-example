...
task :copy_configs_to_shared, :roles => :app do
  ...
  put(File.read('config/gopay.yml'), "#{shared_path}/config/gopay.yml", :mode => 0640)
end
...
task :setup_shared_directories, :roles => :app do
  directory_symlinks
  run "mkdir -p #{shared_path}/assets #{shared_path}/uploads #{shared_path}/backup"
end
...
after "deploy:symlink", "deploy:update_symlinks"